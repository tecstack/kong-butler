pipeline {
  ...
  stages {
    stage('env init') {
      steps {
        sh 'docker run -it -d --name=promise-bulker-dev-ci --hostname=promise-bulker-dev-ci -v /etc/localtime:/etc/localtime:ro -e "PYTHONPATH=/apps/svr/promise-bulker" promise-base /usr/sbin/init'
